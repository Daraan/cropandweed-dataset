    ...
    for bboxes_file in tqdm(sorted(os.listdir(bboxes_dir))):
        ...
        if os.path.exists(os.path.join(bboxes, dataset, bboxes_file)) and image_filter in target:
            ...
            if os.path.exists(image_path):
                ...
                label_ids = (
                    cv2.resize(ids2colors(cv2.imread(label_ids_path, 0), labels), target_size, cv2.INTER_LINEAR)
                ...
                for label_id, count in sorted(label_count.items(), key=lambda item: item[1], reverse=True):
                    ...
                    if label_name is not None:
                        ...
                        image = cv2.putText(
                            image,
                            text,
                            (offset, target_size[1] + 20),
                            font,
                            font_scale,
                            labels.get_label_color(label_id, bgr=True),
                            thickness=font_thickness,
                            lineType=font_line_type,
                        )
...
def main():
    ...
    visualize_annotations(
        args.label_ids, args.bboxes, args.dataset, args.images, args.visualizations, args.target_width, args.filter
    )
